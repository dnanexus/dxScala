...
object DescriptionField extends Enum {
  ...
  // format: off
  val Access, Aliases, Analysis, AppCreatedBy, AppletCreatedBy, ArchivalState, AuthorizedUsers, BillTo, Categories,
      ...
      Properties, Published, RegionalOptions, RootExecution, RunInput, RunSpec, Size, Sponsored, SponsoredUntil,
...
case class EnumFieldSpec[T](key: String, resolve: String => T) extends DescriptionFieldSpec[T] {
  def parse(jsValue: JsValue): T = {
...
object RegionalOptionsFieldSpec extends DescriptionFieldSpec[Vector[RegionOptions]] {
  ...
  override def parse(jsValue: JsValue): Vector[RegionOptions] = {
    jsValue.asJsObject.fields.map {
      ...
      case other =>
        throw new Exception(s"unexpected regionalOptions value ${other}")
    }
  }.toVector
}
...
object DescriptionFieldSpec {
  val Specs: Map[DescriptionField.DescriptionField, DescriptionFieldSpec[_]] = Map(
      ...
      DescriptionField.ArchivalState -> StringFieldSpec("archivalState"),
      ...
      DescriptionField.Size -> LongFieldSpec("size"),
  ...
  def getSpec[T: ClassTag, D <: DescriptionFieldSpec[T]: ClassTag](
      field: DescriptionField.DescriptionField
  ): D = {
    Specs.get(field) match {
      case spec: D => spec
...
trait NewDxObjectDescribe {
  ...
  protected def globalFields: Set[DescriptionField.DescriptionField] =
  ...
  private lazy val allowedFields: Set[DescriptionField.DescriptionField] = {
    globalFields | defaultFields | otherFields
  }

  private lazy val fieldNameToEnum: Map[String, DescriptionField.DescriptionField] =
    allowedFields.map(field => DescriptionFieldSpec.Specs(field).key -> field).toMap
  ...
  def describe(fields: Set[DescriptionField.DescriptionField] = Set.empty,
    ...
    if (allFields.nonEmpty) {
      ...
      cachedDescribeJs ++= response.fields.map {
        case (key, value) => fieldNameToEnum(key) -> value
      }
    }
  }
  ...
  protected def getCachedField[T: ClassTag](
      field: DescriptionField.DescriptionField
  ): Option[T] = {
    if (cachedDescribe.contains(field)) {
      ...
      val value: Option[T] = cachedDescribeJs(field) match {
        case JsNull => None
        case _ =>
          val parser = DescriptionFieldSpec.getSpec[T, DescriptionFieldSpec[T]](field)
          Some(parser.parse(cachedDescribeJs(field)))
      }
  ...
  def getOptionalField[T: ClassTag](field: DescriptionField.DescriptionField): Option[T] = {
  ...
  def getField[T: ClassTag](field: DescriptionField.DescriptionField): T = {
    getOptionalField[T](field).get
  }
}

abstract class BaseDxObjectDescribe extends NewDxObjectDescribe {
  override protected def defaultFields: Set[DescriptionField.DescriptionField] = Set(
      DescriptionField.Created,
      DescriptionField.Modified
  )

  override protected def otherFields: Set[DescriptionField.DescriptionField] = Set(
...
abstract class BaseDxDataObjectDescribe(dxProject: Option[DxProject], dxApi: DxApi = DxApi.get)
  ...
  override protected def defaultFields: Set[DescriptionField.DescriptionField] = {
  ...
  override protected def otherFields: Set[DescriptionField.DescriptionField] = {
  ...
  def setProperties(add: Map[String, String] = Map.empty,
    ...
    val newProperties = currentProperties.view
      ...
      .toMap ++ toAddOrUpdate
    cachedDescribeJs += DescriptionField.Properties -> JsObject(
        newProperties.view
          .mapValues(JsString(_))
          .toMap
    )
    cachedDescribe += DescriptionField.Properties -> Some(newProperties)
    newProperties
  }
}
