...
case class DxApi(version: String = "1.0.0", dxEnv: DXEnvironment = DxApi.defaultDxEnvironment)(
  ...
  def addTags(obj: DxObject, tags: Vector[String], project: Option[DxProject] = None): Unit = {
    ...
    obj match {
      ...
      case dbcluster: DxDbcluster =>
        callObject(DXAPI.dbclusterAddTags[JsonNode], dbcluster.id, fields)
  ...
  def flattenDataObjectsFromJson(jsValue: JsValue): Vector[DxDataObject] = {
    try {
      obj = dataObjectFromJson(jsValue)
      Vector(obj)
    } catch {
      jsValue match {
        case obj: JsObject => {
        ...
        case arr: JsArray => {
          arr.elements.flatMap(flattenDataObjectsFromJson)
        }
        case _ => {
