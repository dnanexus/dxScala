...
class YamlParserSpec extends AnyFlatSpec with Matchers {
  ...
  def withYaml(filename: String)(testFun: YamlValue => Any)(implicit pos: Position): Unit = {
    withYamls(filename) { s =>
      testFun(s.head)
    }
  }

  def withYamls(filename: String)(testFun: Seq[YamlValue] => Any)(implicit pos: Position): Unit = {
    val src = Source.fromFile(getResourceURL(filename))
    Try(src.mkString.parseYamls) match {
  ...
  "The provided YAML parser" should "correctly parse sequences of scalars" in withYaml(
      "/examples/ex1.yaml"
  ) { yaml =>
    yaml should ===(
        YamlArray(YamlString("Mark McGwire"), YamlString("Sammy Sosa"), YamlString("Ken Griffey"))
    )
  }

  it should "correctly parse mappings of scalars to scalars" in withYaml("/examples/ex2.yaml") {
    yaml =>
      yaml should ===(
          YamlObject(YamlString("hr") -> YamlNumber(65),
                     YamlString("avg") -> YamlNumber(0.278),
                     YamlString("rbi") -> YamlNumber(147))
      )
  }

  it should "correctly parse mappings of scalars to sequences" in withYaml("/examples/ex3.yaml") {
    yaml =>
      yaml should ===(
          YamlObject(
              YamlString("american") -> YamlArray(YamlString("Boston Red Sox"),
                                                  YamlString("Detroit Tigers"),
                                                  YamlString("New York Yankees")),
              YamlString("national") -> YamlArray(YamlString("New York Mets"),
  ...
  it should "correctly parse sequences of mappings" in withYaml("/examples/ex4.yaml") { yaml =>
    yaml should ===(
        YamlArray(
            YamlObject(YamlString("name") -> YamlString("Mark McGwire"),
                       YamlString("hr") -> YamlNumber(65),
                       YamlString("avg") -> YamlNumber(0.278)),
            YamlObject(YamlString("name") -> YamlString("Sammy Sosa"),
  ...
  it should "correctly parse sequences of sequences" in withYaml("/examples/ex5.yaml") { yaml =>
    yaml should ===(
        YamlArray(
            YamlArray(YamlString("name"), YamlString("hr"), YamlString("avg")),
            YamlArray(YamlString("Mark McGwire"), YamlNumber(65), YamlNumber(0.278)),
            YamlArray(YamlString("Sammy Sosa"), YamlNumber(63), YamlNumber(0.288))
        )
    )
  }
  ...
  it should "correctly parse mappings of mappings" in withYaml("/examples/ex6.yaml") { yaml =>
    yaml should ===(
        YamlObject(
            YamlString("Mark McGwire") -> YamlObject(YamlString("hr") -> YamlNumber(65),
                                                     YamlString("avg") -> YamlNumber(0.278)),
            YamlString("Sammy Sosa") -> YamlObject(YamlString("hr") -> YamlNumber(63),
                                                   YamlString("avg") -> YamlNumber(0.288))
        )
    )
  }
  ...
  it should "correctly parse aliased nodes" in withYaml("/examples/ex7.yaml") { yaml =>
    yaml should ===(
        YamlObject(
            YamlString("hr") ->
              YamlArray(YamlString("Mark McGwire"), YamlString("Sammy Sosa")),
            YamlString("rbi") ->
              YamlArray(YamlString("Sammy Sosa"), YamlString("Ken Griffey"))
        )
    )
  }
  ...
  it should "correctly parse mappings between sequences" in withYaml("/examples/ex8.yaml") { yaml =>
    yaml should ===(
        YamlObject(
            YamlArray(YamlString("Detroit Tigers"), YamlString("Chicago cubs")) ->
              YamlArray(
                  YamlString("2001-07-23")
//                    YamlDate (
//                        new DateTime(new DateTime("2001-07-23", DateTimeZone.UTC).getMillis,
//                                     DateTimeZone.getDefault())
//                    )
              ),
            YamlArray(YamlString("New York Yankees"), YamlString("Atlanta Braves")) ->
              YamlArray(
                  YamlString("2001-07-02"),
                  YamlString("2001-08-12"),
                  YamlString("2001-08-14")
//                YamlDate(
//                      new DateTime(new DateTime("2001-07-02", DateTimeZone.UTC).getMillis,
//                                   DateTimeZone.getDefault())
//                  ),
//                  YamlDate(
//                      new DateTime(new DateTime("2001-08-12", DateTimeZone.UTC).getMillis,
//                                   DateTimeZone.getDefault())
//                  ),
//                  YamlDate(
//                      new DateTime(new DateTime("2001-08-14", DateTimeZone.UTC).getMillis,
//                                   DateTimeZone.getDefault())
//                  )
              )
        )
  ...
  it should "correctly parse in-line nested mapping" in withYaml("/examples/ex9.yaml") { yaml =>
    yaml should ===(
        YamlArray(
            YamlObject(YamlString("item") -> YamlString("Super Hoop"),
                       YamlString("quantity") -> YamlNumber(1)),
            YamlObject(YamlString("item") -> YamlString("Basketball"),
                       YamlString("quantity") -> YamlNumber(4)),
            YamlObject(YamlString("item") -> YamlString("Big Shoes"),
                       YamlString("quantity") -> YamlNumber(1))
        )
    )
  }
  ...
  it should "correctly replace newlines by spaces in plain scalar" in withYaml(
      "/examples/ex11.yaml"
  ) { yaml =>
  ...
  it should "correctly parse folded new lines" in withYaml("/examples/ex12.yaml") { yaml =>
    yaml should ===(YamlString("""Sammy Sosa completed another fine season with great stats.
                                 |
                                 |  63 Home Runs
    ...
    yaml should ===(
        YamlObject(
            YamlString("canonical") ->
              YamlNumber(12345),
            YamlString("decimal") ->
              // YamlNumber(12345),  // not supported in yaml 1.2
              YamlString("+12345"),
            YamlString("sexagesimal") ->
              // YamlNumber(12345),  // not supported in yaml 1.2
              YamlString("3:25:45"),
            YamlString("octal") ->
              // YamlNumber(12),  // not supported in yaml 1.2
              YamlString("014"),
  ...
  it should "correctly parse large integers" in withYaml("/examples/ex16-2.yaml") { yaml =>
    yaml should ===(
        YamlObject(YamlString("long_canonical") ->
  ...
  it should "correctly parse miscellaneous scalars" in withYaml("/examples/ex18.yaml") { yaml =>
    yaml should ===(
        YamlObject(
            YamlNull ->
              // YamlNull  // looks like ~ no longer means null in yaml 1.2
              YamlString("~"),
            YamlBoolean(true) ->
              YamlString("y"),
            YamlBoolean(false) ->
  ...
  // it looks like timestamps are supported in yaml 1.2
...
//            YamlString("date") ->
//              YamlDate(
//                  new DateTime(new DateTime("2002-12-14", DateTimeZone.UTC).getMillis,
//                               DateTimeZone.getDefault())
//              )
//        )
//    )
//  }
  ...
  it should "correctly parse explicit sets" in withYaml("/examples/ex20.yaml") { yaml =>
    yaml should ===(
        YamlSet(YamlString("Mark McGwire"), YamlString("Sammy Sosa"), YamlString("Ken Griff"))
    )
  }

  // looks like this is no longer supported in yaml 1.2
//  it should "correctly parse explicit ordered mappings" in withYaml("/examples/ex21.yaml") { yaml =>
//    yaml shouldBe YamlObject(YamlString("Mark McGwire") -> YamlNumber(65),
//                             YamlString("Sammy Sosa") -> YamlNumber(63),
//                             YamlString("Ken Griffy") -> YamlNumber(58))
//  }

  it should "correctly parse multiple documents in a stream" in withYamls("/examples/ex22.yaml") {
    yamls =>
      yamls should ===(
          Seq(
              YamlArray(YamlString("Mark McGwire"),
                        YamlString("Sammy Sosa"),
                        YamlString("Ken Griffey")),
              YamlArray(YamlString("Chicago Cubs"), YamlString("St Louis Cardinals"))
          )
      )
  }
}
